...
*/
goog.asserts={};goog.asserts.ENABLE_ASSERTS=goog.DEBUG;goog.asserts.assert=function(){};goog.asserts.patchAssert_=function(){var a=console.assert;a?a.bind||(console.assert=function(){a.apply(console,arguments)}):console.assert=function(){}};goog.asserts.ENABLE_ASSERTS&&(goog.asserts.patchAssert_(),goog.asserts.assert=console.assert.bind(console));function ShakaReceiver(){this.idleTimerId_=this.idle_=this.controlsTimerId_=this.controlsElement_=this.playButton_=this.receiver_=this.player_=this.video_=null;this.idleTimeout_=300}
ShakaReceiver.prototype.init=function(){shaka.polyfill.installAll();var a=document.getElementById("video");goog.asserts.assert(a,"Video element should be available!");this.video_=a;a=this.video_.ui;goog.asserts.assert(a,"UI should be available!");this.player_=a.getPlayer();goog.asserts.assert(this.player_,"Player should be available!");a=document.getElementById("videoContainer");goog.asserts.assert(a,"Video container should be available!");this.controlsElement_=shaka.ui.Utils.getFirstDescendantWithClassName(a,
"shaka-controls-container");this.playButton_=shaka.ui.Utils.getFirstDescendantWithClassName(a,"shaka-play-button-container");this.idle_=document.getElementById("idle");this.video_.addEventListener("play",this.onPlayStateChange_.bind(this));this.video_.addEventListener("pause",this.onPlayStateChange_.bind(this));this.video_.addEventListener("seeking",this.onPlayStateChange_.bind(this));this.video_.addEventListener("emptied",this.onPlayStateChange_.bind(this));this.receiver_=new shaka.cast.CastReceiver(this.video_,
this.player_,this.appDataCallback_.bind(this));this.receiver_.addEventListener("caststatuschanged",this.checkIdle_.bind(this));this.startIdleTimer_()};ShakaReceiver.prototype.appDataCallback_=function(a){a&&ShakaDemoUtils.setupAssetMetadata(a.asset,this.player_)};
