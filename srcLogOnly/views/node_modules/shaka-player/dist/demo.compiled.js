...
[];d.features.includes(shakaAssets.Feature.WEBM)&&f.push("video/webm");d.features.includes(shakaAssets.Feature.MP4)&&f.push("video/mp4");d.features.includes(shakaAssets.Feature.MP2TS)&&f.push("video/mp2t");f.some(function(a){return shakaDemo.support_.media[a]})||(g.disabled=!0);g.disabled||e.disabled||(c=c||g,d.focus&&(c=g))}});c&&(c.selected=!0);var d=shakaDemo.setupOfflineAssets_(),e=document.createElement("option");e.textContent="(custom asset)";a.appendChild(e);a.addEventListener("change",function(){var b=
a.options[a.selectedIndex].asset;document.getElementById("customAsset").style.display=b?"none":"block";shakaDemo.hashShouldChange_()});document.getElementById("loadButton").addEventListener("click",shakaDemo.load);document.getElementById("unloadButton").addEventListener("click",shakaDemo.unload);e=[document.getElementById("licenseServerInput"),document.getElementById("manifestInput"),document.getElementById("certificateInput")];e=$jscomp.makeIterator(e);for(var f=e.next();!f.done;f=e.next())f=f.value,
f.addEventListener("input",shakaDemo.onAssetInput_),f.addEventListener("keydown",shakaDemo.onAssetKeyDown_);return d};shakaDemo.onAssetKeyDown_=function(a){"Enter"==a.key&&shakaDemo.load()};shakaDemo.onAssetInput_=function(a){shakaDemo.hashShouldChange_()};shakaDemo.requestCertificate_=function(a){return shakaDemo.player_.getNetworkingEngine().request(shaka.net.NetworkingEngine.RequestType.APP,{uris:[a]}).promise.then(function(a){return a.data})};
shakaDemo.configureCertificate_=function(a){var b=shakaDemo.player_,c=b.getConfiguration(),d={},e;for(e in c.drm.advanced)d[e]={serverCertificate:new Uint8Array(a)};b.configure({drm:{advanced:d}})};
...
manifestUri:"https://bitdash-a.akamaihd.net/content/MI201109210084_1/m3u8s/f08e80da-bf1d-4e3d-8899-f0f6155f6efa.m3u8",iconUri:"https://storage.googleapis.com/shaka-asset-icons/art_of_motion.png",shortName:"Art of Motion",encoder:shakaAssets.Encoder.BITCODIN,source:shakaAssets.Source.BITCODIN,drm:[],features:[shakaAssets.Feature.HIGH_DEFINITION,shakaAssets.Feature.HLS,shakaAssets.Feature.MP2TS,shakaAssets.Feature.OFFLINE]},{name:"Sintel (HLS, TS, 4k)",manifestUri:"https://bitdash-a.akamaihd.net/content/sintel/hls/playlist.m3u8",
...
a.responseFilter&&c.registerResponseFilter(a.responseFilter);a.extraConfig&&b.configure(a.extraConfig)},addLicenseRequestHeaders_:function(a,b,c){if(b==shaka.net.NetworkingEngine.RequestType.LICENSE)for(var d in a)c.headers[d]=a[d]},isTsContent:function(a){return(a=a.getVariantTracks().filter(function(a){return 1==a.active})[0])?"video/mp2t"==a.mimeType:!1},createAssetButtons:function(a,b,c,d,e){var f=[],g={};b=$jscomp.makeIterator(b);for(var h=b.next();!h.done;g={asset:g.asset,button:g.button},h=
b.next())g.asset=h.value,g.button=document.createElement("div"),d(g.button,g.asset),g.button.onclick=function(a){return function(){e(a.asset);for(var b=$jscomp.makeIterator(f),c=b.next();!c.done;c=b.next())c.value.removeAttribute("selected");a.button.setAttribute("selected","")}}(g),a.appendChild(g.button),f.push(g.button),g.asset==c&&g.button.setAttribute("selected","")},runThroughHashParams:function(a,b){var c=[],d=function(b,d){c.push(d);a(b,d)};d("audiolang","preferredAudioLanguage");d("textlang",
"preferredTextLanguage");d("channels","preferredAudioChannelCount");c.push("abr.enabled");c.push("streaming.jumpLargeGaps");var e=[],f=[],g=function(a){for(var b in a){e.includes(b)&&!f.includes(b)&&f.push(b);e.push(b);var c=a[b];"number"!=typeof c&&"string"!=typeof c&&"boolean"!=typeof c&&g(c)}};g(b);var h=function(b,d){for(var e in b){var g=e,k=d+e;if(!c.includes(k)){f.includes(e)&&(g=k);var l=b[e];"number"==typeof l||"string"==typeof l||"boolean"==typeof l?a(g,k):h(l,k+".")}}};h(b,"")},browserSupportsUncompiledMode:function(){try{return eval("async ()=>{}"),
!0}catch(a){return!1}}};shakaDemo=shakaDemo||{};
...
shakaDemo.setupLogging_();shakaDemo.preBrowserCheckParams_(b);window.addEventListener("error",function(a){shakaDemo.onError_(a.error)});shaka.Player.isBrowserSupported()?(navigator.serviceWorker&&(console.debug("Registering service worker."),navigator.serviceWorker.register("service_worker.js").then(function(a){console.debug("Service worker registered!",a.scope)})["catch"](function(a){console.error("Service worker registration failed!",a)})),a=function(a){a=document.getElementById("version");var b=
a.textContent;b=b.replace(" (offline)","");navigator.onLine||(b+=" (offline)");a.textContent=b;shakaDemo.computeDisabledAssets()},window.addEventListener("online",a),window.addEventListener("offline",a),a(null),shaka.Player.probeSupport().then(function(a){shakaDemo.support_=a;var c=document.getElementById("video");a=c.parentElement;var e=c.ui,f=e.getPlayer();shakaDemo.castProxy_=e.getControls().getCastProxy();shakaDemo.video_=shakaDemo.castProxy_.getVideo();shakaDemo.player_=shakaDemo.castProxy_.getPlayer();
shakaDemo.player_.addEventListener("error",shakaDemo.onErrorEvent_);shakaDemo.localVideo_=c;shakaDemo.localPlayer_=f;shakaDemo.localVideo_.poster=shakaDemo.mainPoster_;c=shakaDemo.setupAssets_();shakaDemo.setupOffline_();shakaDemo.setupInfo_();shakaDemo.setupConfiguration_();null!=a&&(shakaDemo.controls_=e.getControls(),a=document.getElementById("preferredUILanguage").value,shakaDemo.controls_.getLocalization().changeLocale([a]),shakaDemo.controls_.addEventListener("error",function(a){shakaDemo.onError_(a.errorDetails)}),
shakaDemo.controls_.addEventListener("caststatuschanged",function(a){shakaDemo.onCastStatusChange_(a.newStatus)}),shakaDemo.controls_.setEnabledShakaControls(!1),c["catch"](function(a){shakaDemo.onError_(a)}).then(function(){shakaDemo.postBrowserCheckParams_(b);window.addEventListener("hashchange",shakaDemo.updateFromHash_)}))})["catch"](function(a){shakaDemo.onError_(a)})):(a=document.getElementById("errorDisplayLink"),void 0===a.textContent?a.innerText="Your browser is not supported!":a.textContent=
"Your browser is not supported!",document.getElementById("loadButton").disabled=!0,document.getElementById("unloadButton").disabled=!0,document.getElementById("errorDisplayCloseButton").style.display="none",a.style.fontSize="250%",a.href="#",a.style.pointerEvents="none",a.style.textDecoration="none",a.style.cursor="default",a.onclick=function(){return!1},document.getElementById("errorDisplay").style.display="block")};
shakaDemo.getParams_=function(){var a=location.search.substr(1);a=a?a.split(";"):[];var b=location.hash.substr(1);b=b?b.split(";"):[];a=a.concat(b);b={};for(var c=0;c<a.length;++c){var d=a[c].split("=");b[d[0]]=d.slice(1).join("=")}return b};
...
c),(c=document.getElementById("licenseServerInput").value)&&a.push("license="+c),(c=document.getElementById("certificateInput").value)&&a.push("certificate="+c)):a.push("asset="+d[d.selectedIndex].asset.manifestUri);document.getElementById("smallGapLimit").value.length&&a.push("smallGapLimit="+document.getElementById("smallGapLimit").value);document.getElementById("jumpLargeGaps").checked&&a.push("jumpLargeGaps");c=document.getElementById("preferredAudioLanguage").value;e=document.getElementById("preferredTextLanguage").value;
...
shakaDemo.setupOfflineAssets_=function(){var a=shaka.offline.Storage;if(!a.support())return document.getElementById("offlineSection").style.display="none",Promise.resolve();var b=document.getElementById("assetList");if(shakaDemo.offlineOptGroup_)var c=shakaDemo.offlineOptGroup_;else c=document.createElement("optgroup"),shakaDemo.offlineOptGroup_=c,c.label="Offline",b.appendChild(c);var d=new a(shakaDemo.localPlayer_);return d.list().then(function(a){a.forEach(function(a){for(var e=0;e<b.options.length;e++){var d=
b.options[e];if(d.asset&&d.asset.manifestUri==a.originalManifestUri){d.isStored=!0;break}}e={manifestUri:a.offlineUri};d=document.createElement("option");d.textContent=a.appMetadata?a.appMetadata.name:"";d.asset=e;d.storedContent=a;c.appendChild(d)});shakaDemo.updateButtons_(!0);return d.destroy()})};
